...
public:
	TLS();
	static bool handshake(const char* psk_value, const char* psk_id, WiFiClient client);
	static void sendAppData(char* data, int dataSize, WiFiClient client, const char* key);
	...
	static void sendpacket(uint8_t* packet, int packetSize, WiFiClient client);
